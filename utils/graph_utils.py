...
def print_graph_stats(G): # 输出图的边和节点数
...
def construct_adjacency(G, id2idx, sparse=False, file_path=None):
    ...
    构造邻接矩阵
    :param G: 由G.json构成的图
    :param id2idx: id到index的映射
    :param sparse: 是否为稀疏矩阵，bool
    :param file_path: 默认为空
    :return:
    '''
    ...
    if file_path: # 将邻接矩阵存储起来，以(id1, id2)的方式存储
    ...
    if format == 'matrix':
        ...
        with open(path) as file:
            for line in file:
                src, trg = line.strip().split()
                # print(src, trg)
                if id2idx_src: # id -> idx，说明gt中存储的是id之间的映射
